...
pub fn save_problem_files(
    ...
    let main_cpp = problem_path.join("main.cpp");
    if !main_cpp.exists() || force {
        // No template configured means no main.cpp is generated by default.
        if let Some(template) = crate::config::load()?.template() {
            std::fs::write(&main_cpp, template)?;
        }
    }
